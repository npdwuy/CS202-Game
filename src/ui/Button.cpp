...
{
    m_nineSlice = std::make_unique<NineSlice>(texture, size);
    m_nineSlice->setOrigin(size.x / 2.f, size.y / 2.f);
    m_nineSlice->setPosition(position);
    m_nineSlice->setColor(m_normalColor);
...
void Button::update(sf::Vector2f mousePos) {
    ...
    if (m_isHovered) {
        ...
        if (m_nineSlice) {
            m_nineSlice->setColor(m_hoverColor);
            m_nineSlice->setScale(1.08f, 1.08f);
        }
    } else {
        m_shape.setFillColor(m_normalColor);
...
void Button::setColors(sf::Color normalColor, sf::Color hoverColor, sf::Color textColor) {
    ...
    if (m_nineSlice) {
        m_nineSlice->setColor(currentColor);
    }
}
